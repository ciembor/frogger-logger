...
(function() {
  ...
  FL.isMessageForThisClient = function(message) {
    if ('client_id' in message) {
      if (FroggerLogger.ID !== message.client_id) {
        return false;
      }
    }
    return true;
  ...
  FL.onOpen = function(ws) {
    message = JSON.stringify({
      client_id: FroggerLogger.ID, 
      status: 'ready'
    });
    return function() { 
      ws.send(message);
  ...
  FL.handleMessage = function(message) {
    ...
    if (this.isMessageValid(message)) {
      if (this.isMessageForThisClient(message)) {
